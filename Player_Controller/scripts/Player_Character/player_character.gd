...
#var player_name: String = "":
	#set(p_name):
		...
		#nametag.text = p_name
#
#var last_hit_player: CharacterBody3D
...
var fall_gravity = ProjectSettings.get_setting("physics/3d/default_gravity")
...
func _input(event: InputEvent) -> void:
	...
	# NOTE: Aim might be needed to be moved to Input for netfox
...
# NOTE: formerly _physics_process
func _rollback_tick(_delta, tick, is_fresh):
	sprint_replenish(_delta)
	#lean_collision()
	
	# NOTE: Not good. Casuses weird interpolation... Do not send camera movements. 
	#if event is InputEventMouseMotion:
		#var MouseEvent = event.relative * mouse_sensitivity
		#camera_look(MouseEvent)
		#camera_look(input.aim * mouse_sensitivity)
	...
	# NOTE: Replaced the following line to Netfox
	var input_dir = _player_input.movement
